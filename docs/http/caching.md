...
HTTPS 客户端/服务器通信加密，代理只能隧道响应，不能充当缓存
...
- 如果响应的年龄*小于* 1 周，则响应是新鲜的
- 如果响应的年龄*超过* 1 周，则响应是陈旧的

> 当响应存储在共享缓存中时，有必要通知客户端响应的年龄。如果共享缓存将响应存储了
...
> 在 23:22:22，响应变得陈旧，无法重用缓存。客户端发送带有
...
收到该响应后，客户端将存储的陈旧响应恢复为新鲜的，并可以在剩余的
1 小时内重复使用
...
> 如果该响应是陈旧的，则客户端获取缓存响应头 `ETag` 的值，并将其放入
...
要想获得网络平台完整功能优势，最好结合使用 `no-cache` 和 `private`
...
为了从界面异常中恢复或更新到最新版本的资源，浏览器为用户提供了重新加载功能
